...
## SSO that drives roles, not just login

The client already had Authentik in front of every other internal tool, with groups that mapped cleanly onto the four roles the platform needs (admin, question creator, reviewer, exam-taker). I wired the platform's auth to Authentik over OAuth2/OIDC for login and SCIM for user and group provisioning, then read roles directly off the SCIM group membership. Adding a new reviewer is a group change in Authentik, not a ticket to me. There is no second user table to drift, no manual onboarding step, and no out-of-band group sync to break.

## Audit-ready exports, not stitched screenshots

Compliance teams have to prove, on demand, who was certified, on what, when, and by which reviewer. The platform exports that record as both Excel and PDF, generated from the same query so the two formats cannot disagree. Excel goes to the spreadsheets the auditors actually open, PDF goes into the binders they actually file. The export work runs through the same Postgres job queue as the rest, so a large period export does not block the request thread and a failed run is something an admin can retry without losing position.
